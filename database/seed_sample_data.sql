...
-- ============================================================================
-- Usage (from Git Bash, after schema.sql has been run):
--   docker exec -i reconciliation_postgres psql -U reconciliation_user -d reconciliation_db < database/seed_sample_data.sql
-- ============================================================================

-- 1. Bank format configs -- must be populated before bank_transactions can be inserted
...
-- 3. Bot transactions (sample data entered via bot / Microsoft Forms interim, see ADR-0008)
...
-- 4. Bank file batch (example of one "processed" XLSX file)
...
(
    ...
    'e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b85' /* sample hash, not a real one */,
    'RHB', 'PROCESSED', 3, now()
);

-- 5. Bank transactions (example parsing output, matches the format in Technical Design Document Section 5)
...
(
    ...
    'JUNAEDI NURJANNAH', 30000, 'hash_sample_001'
),
(
...
(
    ...
    'MOHD IKHWAN BIN MOBIN', 70850, 'hash_sample_003'
);

-- 6. Finance users (example, 2 roles per the MVP-Solo scope -- see Solo Delivery Plan)
...
-- 7. Example match result (Level 1, exact reference match -- see Technical Design Document Section 15)
